...
void restartGame() {
    // Reset game variables, scores, and any other necessary state
    speed = 0.0f;
    healthCount = 3;
    score = 0;
    _move = 0.0f;
...
void drawText(const char* text, int length, float x, float y, float yOffset = 0.0f) {
...
void drawStart(const char* text, int length, float x, float y, float yOffset = 0.0f) {
    glMatrixMode(GL_PROJECTION);
    double* matrix = new double[16];
    glGetDoublev(GL_PROJECTION_MATRIX, matrix);
    glMatrixMode(GL_MODELVIEW);
...
void drawScene() {
    ...
    glColor3d(1, 0, 0);
    glLoadIdentity();
    glMatrixMode(GL_MODELVIEW);  // Fix the typo here
    _stars -= 0.001f;
    ...
    if (gameState == GAME_RUNNING) {
        // Display prompt to press 'Up' key at the bottom of the screen
        glColor3d(0.5, 0.5, 0.5); // Warna abu-abu (R=0.5, G=0.5, B=0.5)
        string startMessage = "Press 'Up' key to start";
        drawStart(startMessage.data(), startMessage.size(), -0.9f, -0.95f);
    }
    ...
    if (healthCount == 0) {
        ...
        drawText(gameOverText.data(), gameOverText.size(), -0.3f, 0.8f);
        ...
        string text = "Score: ";
        convert << score;
        text = text + convert.str();
        drawText(text.data(), text.size(), -0.2f, 0.6f);

        // Display restart message
        glColor3d(1, 1, 1);
        string restartMessage = "Press 'R' to Restart";
        drawText(restartMessage.data(), restartMessage.size(), -0.5f, 0.2f);
        ...
        convert << score;
        text = convert.str();
        drawText(text.data(), text.size(), 0.0f, 0.6f);
    }
    ...
    else if ((_move > -1.0f && _move < 0.0f) && (_obstacle1 < -1.45f && _obstacle1 > -1.8f) && random == 4) {
        Sleep(150);
        healthCount--;
        obstacleFlag = 0;
        _obstacle1 = 0.0f;			// Pengurangan Skor
    }
    else if ((_move < 1.0f && _move > 0.0f) && (_obstacle1 < -1.45f && _obstacle1 > -1.8f) && random == 5) {
        // Tabrakan dengan Penghalang di Sisi Kanan
        glutDisplayFunc(drawScene);
        Sleep(150);
        healthCount--;
...
void restartIfGameOver() {
    if (gameState == GAME_OVER) {
        // Izinkan tombol 'r' untuk memulai ulang permainan
...
void update(int value) {
    ...
    restartIfGameOver();
    glutTimerFunc(6, update, 0); // Memberitahu GLUT untuk memanggil fungsi update lagi dalam 10 milidetik
}
